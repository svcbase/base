...
func SQL_datediff(startfield, endfield string) (ss string) { /* days */
	sfld, efld := startfield, endfield
	if sfld == "now" {
		sfld = SQL_now()
	}
	if efld == "now" {
	...
	switch DB_type {
	case SQLite:
		ss = "cast(julianday(" + sfld + ")-julianday(" + efld + ") as int)"
	case MySQL:
		ss = "datediff(" + sfld + "," + efld + ")"
	}
	return
}
