				...
				if v, ok := mapParam[key]; ok && (v == value) {
					...
					case "mysql":
						...
						if db_pswd, ok = mapParam["db_pswd"]; ok && (len(db_pswd) > 0) {
							db_pswd = DecodeByKey(db_pswd, KeyGen(TotalASCII(preferredMAC), 1111))
						}
...
func Pages(nrows int) (pages int) {
	pages = (nrows + PAGE_ROWS - 1) / PAGE_ROWS
	return
}
func Paging(nrows int, cur_page string) (ipage, pages int) {
	pages = (nrows + PAGE_ROWS - 1) / PAGE_ROWS
	ipage = 1
